<template>
  <div class="navbar bg-base-200 px-10">
    <div class="navbar-start dropdown">
      ...
      <ul tabindex="0" class="menu menu-sm dropdown-content bg-base-200 rounded-box z-[1] mt-3 w-52 p-3 shadow">
    ...
    <div class="navbar-end gap-4">
      <label class="grid cursor-pointer place-items-center">
        <input type="checkbox" value="synthwave" class="toggle theme-controller bg-base-content col-span-2 col-start-1 row-start-1" @change="toggleTheme" />
        ...
        <div class="dropdown dropdown-end">
          <div tabindex="0" role="button" class="btn btn-ghost btn-circle avatar">
            <div class="w-10 rounded-full relative">
              ...
              <img v-if="avatarUrl" :src="avatarUrl" alt="User Avatar" class="w-10 h-10 rounded-full object-cover absolute inset-0" />
            </div>
          </div>
...
export default {
  setup() {
    ...
    const goToBestPlayers = () => navigateTo('/best_players');
    const goToSearchHistory = () => navigateTo('/search_history');
    ...
    onMounted(async () => {
      if (user.value) {
        const { data } = await client.from('profiles').select('avatar_url').eq('id', user.value.id).single();
        if (data && data.avatar_url) {
          const { data: publicUrl } = client.storage.from('avatars').getPublicUrl(data.avatar_url);
